...
void main()
{
	...
	for (int i=0;i<3;i++) {
		...
		float r = length(vertexPos.xyz);
		float theta = acos(vertexPos.z/r);
		float phi = atan(vertexPos.y, vertexPos.x);

		// //fractal noise
		vec3 pNorm1 = vertexPos.xyz;
		float n1 = noise(pNorm1, 7, 3., 0.7, 1); 
		vec3 pNorm2 = vertexPos.xzy;
		float n2 = noise(pNorm2, 7, 3., 0.7, 1); 
		...
		gl_Position =  uv_modelViewProjectionMatrix *vec4(vertexPos.xyz, 1./(neutrinoLength*rad));
		texcoord = vec2(0, 1);
		toDraw = j;
		EmitVertex();
		j += 1;
